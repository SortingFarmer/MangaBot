...
module.exports = {
	...
	async execute(interaction) {
		...
		let savedTemplates = SearchTemplate.findAll({
			where: { userId: interaction.user.id },
		});
		let templates = [];
		...
		search.setCustomId(
			`searchTemplate_${interaction.user.id}_${
		...
		filter.setCustomId(
			`filter.1_${interaction.user.id}_${
				Math.floor(Date.now() / 1000) + expire
			}`
		);
			...
			embeds: [
				{
					title: "Browse",
					description:
						"Welcome to the browse menu. Here you can browse through a list of mangas.\n" +
					...
					fields: [
						{
							name: "**Default filter:**",
							value:
								`The default filter filters by Highest raing.\n` +
						...
						},
						{
							...
							value:
								`Here are all of the options you can customize:\n` +
