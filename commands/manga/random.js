...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			stats = await fetchStatisticsData(mangadex.api, [
				result.data.data.id,
			]);
			chapters = await fetchChapterData(
				mangadex.api,
				result.data.data.id
			);
		} catch (error) {
		...
		comments.setURL(
			`${mangadex.forum}/${
				stats.data.statistics[result.data.data.id].comments?.threadId
			}`
		);

		const row = new ActionRowBuilder();
		stats.data.statistics[result.data.data.id].comments == null
			? row.addComponents(random, follow, link)
			: row.addComponents(random, follow, link, comments);
