...
module.exports = {
	...
	mangaEmbed: function (manga, ratings, chapterData = undefined) {
					...
					result[vol][chap].forEach((item, index) => {
						const spacer =
							chapIndex === chapters.length - 1
								...
								: char.replyNone;
						const prefix =
							index === result[vol][chap].length - 1
								? char.reply
								: char.replyMore;
						chapterString += `${spacer}${prefix} ${item.lang} - ${
							/*char.people} ${item.group.join(", ")}, ${*/ char.person
						} ${item.user.join(", ")}\n`;
					});
				});
			});
		...
		fields.push({
			name: "**Chapters**",
			value:
				chapterString.length > 1024
					? chapterString.substring(0, 1021) + "..."
